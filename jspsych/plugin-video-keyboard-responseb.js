var jsPsychVideoKeyboardResponseb = (function (jspsych) {
  'use strict';

  const info = {
      name: "video-keyboard-responseb",
      parameters: {
   ...
   */
  class VideoKeyboardResponsebPlugin {
      constructor(jsPsych) {
      ...
      `);
          ...
          // store response
          var response = {
              rt: [], // ww changed from null to []
              key: [], // ww changed from null to []
          };
          ...
          const end_trial = () => {
              ...
              var trial_data = {
                  rt: JSON.stringify(response.rt), // ww add JSON.stringify
                  stimulus: trial.stimulus,
                  response: JSON.stringify(response.key),// ww add JSON.stringify
              };
          ...
          if (trial.choices != "NO_KEYS" && trial.response_allowed_while_playing) {
              this.jsPsych.pluginAPI.getKeyboardResponse({
                  ...
                  valid_responses: trial.choices,
                  rt_method: "performance",
                  persist: true,   // ww changed from false to true
                  allow_held_key: false,
              });
          }
  ...
  VideoKeyboardResponsebPlugin.info = info;

  return VideoKeyboardResponsebPlugin;
